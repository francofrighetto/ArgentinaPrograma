import { Task } from "./Task"
// los tipos de datos estan definidos en la interface Task
// si quiero poner otro atributo en los distintos registros, o un valor que no corresponde
// nos da error, porque hay q tener consistencia con la interface
export const TASKS: Task[] =[
    {
        id:1,
        text:"Terminar primer modulo de Angular",
        day:'Agosto 5 a las 12:00',
        reminder:true
    },
    {
        id:2,
        text:"Hacer las compras para la cena",
        day:'Agosto 5 a las 17:00',
        reminder:true
    },
    {
        id:3,
        text:"Investigar sobre Boostrap",
        day:'Agosto 5 a las 19:00',
        reminder:false
    },
    {
        id:4,
        text:"Leer mi libro favorito",
        day:'Agosto 5 a las 21:00',
        reminder:false
    }
]